          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border-none text-[11px] font-bold w-48 focus:ring-2 focus:ring-primary/20 transition-all"
              />
            </div>
            <button className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 rounded-xl text-[11px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-100 transition-all">
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-24 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400">No payment history found yet.</p>
                  </td>
                </tr>
              ) : (
                payments.map(p => (
                  <tr key={p.id} className="group hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <span className="text-[11px] font-mono font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-md">{p.externalReference}</span>
                    </td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6">
          ...
          <div>
             ...
             <p className="text-2xl font-black text-slate-900">{payments.filter(p => p.status === 'APPROVED').length}</p>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6">
          ...
          <div>
             ...
             <p className="text-2xl font-black text-slate-900">{payments.filter(p => !['APPROVED', 'REJECTED'].includes(p.status)).length}</p>
          </div>
        </div>
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6">
          ...
          <div>
             ...
             <p className="text-2xl font-black text-slate-900">{payments.filter(p => p.status === 'REJECTED').length}</p>
          </div>
        </div>
      </div>
    </div>
